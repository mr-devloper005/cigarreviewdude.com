...
import { ArrowUpRight, BookOpen, Bookmark, Building2, ChevronDown, Download, FileText, Globe, MapPin, Phone, Search, UserRound } from 'lucide-react'
...
const cardBase = 'group block rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] transition duration-500 hover:-translate-y-1 hover:shadow-[0_22px_54px_rgba(82,0,128,0.10)]'
    ...
    <EditableSiteShell>
      <main style={taskThemeStyle(task)} className="min-h-screen bg-[var(--tk-bg)] text-[var(--tk-text)]">
        <header className="relative overflow-hidden">
          ...
          <div className="relative mx-auto max-w-[var(--editable-container)] px-5 py-16 sm:px-6 sm:py-24 lg:px-8 lg:py-28">
            <EditableReveal>
              <div className="inline-flex items-center gap-2 rounded-full border border-[var(--tk-line)] bg-[var(--tk-surface)] px-3.5 py-1.5 text-[0.72rem] font-medium uppercase tracking-[0.18em] text-[var(--tk-accent)]">
                {label}
              </div>
            </EditableReveal>
            <EditableReveal index={1}>
              <h1 className="editable-display mt-6 max-w-3xl text-balance text-[2.5rem] leading-[1.05] sm:text-[3.5rem] lg:text-[4.5rem]">
                {voice?.headline || `Browse ${label}`}
              </h1>
            </EditableReveal>
            <EditableReveal index={2}>
              <p className="mt-6 max-w-2xl text-[1.05rem] leading-8 text-[var(--tk-muted)] sm:text-[1.15rem]">
                ...
                <form action={basePath} className="flex items-center gap-2.5">
                  <div className="relative">
                    <select
                      ...
                      className="h-11 appearance-none rounded-[0.75rem] border border-[var(--tk-line)] bg-[var(--tk-surface)] pl-4 pr-10 text-sm font-medium text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
        ...
        {task === 'pdf' ? (
          <div className="mx-auto max-w-[var(--editable-container)] px-5 pb-8 sm:px-6 lg:px-8">
            <Ads slot="header" size={pickRandom(getSlotSizes('header'))} showLabel className="mx-auto w-full" />
          </div>
        ) : null}

        <section className="mx-auto max-w-[var(--editable-container)] px-5 pb-24 pt-6 sm:px-6 lg:px-8">
          {posts.length ? (
            <div className={taskGrid[task]}>
              ...
              {task === 'listing' && posts.length ? (
                <EditableReveal index={2}>
                  <div className="rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-6 xl:col-span-2">
            ...
            <div className="mx-auto max-w-xl rounded-[var(--tk-radius)] border border-dashed border-[var(--tk-line)] bg-[var(--tk-surface)] px-8 py-16 text-center">
              ...
              <p className="mt-2 text-sm leading-6 text-[var(--tk-muted)]">Try another category, or check back — {label.toLowerCase()} entries are added by hand.</p>
            </div>
          )}

          {posts.length ? (
            <nav className="mt-16 flex items-center justify-center gap-3 text-sm">
              {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-[0.75rem] border border-[var(--tk-line)] px-5 py-2.5 font-medium transition hover:border-[var(--tk-accent)]">Previous</Link> : null}
              <span className="rounded-[0.75rem] border border-[var(--tk-line)] bg-[var(--tk-surface)] px-5 py-2.5 font-medium text-[var(--tk-muted)]">Page {page} of {pagination.totalPages || 1}</span>
              {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-[0.75rem] border border-[var(--tk-line)] px-5 py-2.5 font-medium transition hover:border-[var(--tk-accent)]">Next</Link> : null}
            </nav>
          ) : null}
        </section>
      </main>
...
function CardArrow({ label }: { label: string }) {
  return (
    <span className="mt-5 inline-flex items-center gap-1.5 text-sm font-medium text-[var(--tk-text)] underline decoration-[3px] decoration-[var(--tk-accent)]/60 underline-offset-[6px] transition group-hover:decoration-[var(--tk-accent)]">
      {label}
  ...
  return (
    <Link href={href} className={`${cardBase} overflow-hidden`}>
      ...
      <div className="p-6 sm:p-7">
        <div className="flex items-center gap-2 text-[0.72rem] font-medium uppercase tracking-[0.18em] text-[var(--tk-accent)]">
          <span>{category}</span>
          <span className="text-[var(--tk-muted)]">· No. {String(index + 1).padStart(2, '0')}</span>
        </div>
        <h2 className="editable-display mt-3 text-[1.75rem] leading-[1.15]">{post.title}</h2>
        <p className="mt-3 line-clamp-3 text-[15px] leading-7 text-[var(--tk-muted)]">{getSummary(post)}</p>
        <CardArrow label="Read the piece" />
      </div>
    </Link>
  )
}
  ...
  return (
    <Link href={href} className={`${cardBase} flex items-center gap-5 p-5 sm:p-6`}>
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[0.7rem] font-medium uppercase tracking-[0.16em] text-[var(--tk-accent)]">Directory entry</p>
        <h2 className="editable-display mt-1 truncate text-[1.5rem] leading-[1.15]">{post.title}</h2>
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col p-6 sm:p-7`}>
      <div className="flex items-start justify-between gap-4">
        <span className="editable-display text-[2.5rem] leading-none text-[var(--tk-accent)]">{price || 'Open offer'}</span>
        {condition ? <span className="rounded-full bg-[var(--tk-accent-soft)] px-3 py-1 text-[0.7rem] font-medium uppercase tracking-[0.14em] text-[var(--tk-accent)]">{condition}</span> : null}
      </div>
      <h2 className="editable-display mt-5 text-[1.5rem] leading-[1.15]">{post.title}</h2>
  ...
  return (
    <Link href={href} className="group mb-5 block break-inside-avoid overflow-hidden rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] transition duration-500 hover:-translate-y-1">
      <div className={`relative overflow-hidden ${index % 3 === 0 ? 'aspect-[3/4]' : 'aspect-[4/3]'}`}>
        ...
        <div className="absolute inset-0 bg-[linear-gradient(180deg,transparent_45%,rgba(18,14,0,0.85))] opacity-80 transition group-hover:opacity-100" />
        <div className="absolute inset-x-0 bottom-0 p-5">
          <h2 className="editable-display line-clamp-2 text-[1.35rem] leading-[1.15] text-white">{post.title}</h2>
...
function BookmarkArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex gap-4 p-6`}>
      <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-[0.75rem] bg-[var(--tk-accent-soft)] text-[var(--tk-accent)]">
        <Bookmark className="h-5 w-5" />
      </div>
      <div className="min-w-0 flex-1">
        <span className="text-[0.7rem] font-medium uppercase tracking-[0.16em] text-[var(--tk-muted)]">Saved · {String(index + 1).padStart(2, '0')}</span>
        <h2 className="editable-display mt-1 text-[1.35rem] leading-[1.15]">{post.title}</h2>
        <p className="mt-2 line-clamp-2 text-sm leading-6 text-[var(--tk-muted)]">{getSummary(post)}</p>
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col p-7`}>
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="rounded-full border border-[var(--tk-line)] px-3 py-1 text-[0.7rem] font-medium uppercase tracking-[0.14em] text-[var(--tk-muted)]">{category}</span>
      </div>
      <h2 className="editable-display mt-6 text-[1.5rem] leading-[1.2]">{post.title}</h2>
      <p className="mt-3 line-clamp-3 flex-1 text-sm leading-7 text-[var(--tk-muted)]">{getSummary(post)}</p>
      <span className="mt-6 inline-flex items-center gap-1.5 text-sm font-medium text-[var(--tk-text)] underline decoration-[3px] decoration-[var(--tk-accent)]/60 underline-offset-[6px]">
        Download the brief <Download className="h-4 w-4" />
      </span>
    </Link>
  )
}
...
function ProfileArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col items-center p-7 text-center`}>
      ...
      <h2 className="editable-display mt-5 text-[1.35rem] leading-[1.15]">{post.title}</h2>
      ...
      <span className="mt-4 inline-flex items-center gap-1.5 text-xs font-medium text-[var(--tk-accent)]">Open profile <BookOpen className="h-3.5 w-3.5" /></span>
    </Link>
  )
}
