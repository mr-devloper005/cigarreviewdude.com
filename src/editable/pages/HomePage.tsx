...
export default async function HomePage() {
  ...
  const libraryPosts = uniquePosts(taskFeed.find(({ task }) => task.key === 'pdf')?.posts || [])
  const baseUrl = SITE_CONFIG.baseUrl.replace(/\/$/, '')
  const sharedProps = { primaryTask, primaryRoute, posts: primaryPosts, timeSections }
  ...
  return (
    <EditableSiteShell>
      <main className="editable-enter">
        ...
        <EditableHomeHero {...sharedProps} />

        <div className="mx-auto w-full max-w-[var(--editable-container)] px-5 py-6 sm:px-6 lg:px-8">
          <Ads slot="header" size={pickRandom(getSlotSizes('header'))} showLabel eager className="mx-auto w-full" />
        </div>

        <EditableStoryRail {...sharedProps} />
        <EditableMagazineSplit {...sharedProps} />
        <EditableLibraryStrip posts={libraryPosts} />
        <EditableTimeCollections {...sharedProps} />
        <EditableStatsBand {...sharedProps} />
        <EditableHomeCta />
      </main>
    </EditableSiteShell>
  )
